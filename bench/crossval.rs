...
fn main() {
    ...
    emit("vec_u32_5", &bcs::to_bytes(&vec![100u32, 200, 300, 400, 500]).unwrap());
    emit("vec_u64_1", &bcs::to_bytes(&vec![0xDEADBEEFu64]).unwrap());
    emit("vec_string_2", &bcs::to_bytes(&vec!["abc".to_string(), "def".to_string()]).unwrap());
    emit("vec_vec_u8", &bcs::to_bytes(&vec![vec![1u8, 2], vec![3u8, 4, 5]]).unwrap());

    // ── Fixed arrays ──────────────────────────────────────────────
    emit("arr_u8_4", &bcs::to_bytes(&[0xAAu8, 0xBB, 0xCC, 0xDD]).unwrap());
    emit("arr_u16_3", &bcs::to_bytes(&[1u16, 2, 3]).unwrap());
    emit("arr_u32_2", &bcs::to_bytes(&[0x12345678u32, 0xABCDEF01]).unwrap());
    ...
    emit("opt_some_u64", &bcs::to_bytes(&Some(0xCAFEBABEu64)).unwrap());
    emit("opt_none_u64", &bcs::to_bytes(&Option::<u64>::None).unwrap());
    emit("opt_some_string", &bcs::to_bytes(&Some("hi".to_string())).unwrap());
    emit("opt_none_string", &bcs::to_bytes(&Option::<String>::None).unwrap());
    emit("opt_opt_some", &bcs::to_bytes(&Some(Some(99u8))).unwrap());
    emit("opt_opt_none_inner", &bcs::to_bytes(&Some(Option::<u8>::None)).unwrap());
    emit("opt_opt_none_outer", &bcs::to_bytes(&Option::<Option<u8>>::None).unwrap());

    // ── Structs ───────────────────────────────────────────────────
    emit("simple_struct", &bcs::to_bytes(&SimpleStruct {
        a: 42, b: true, c: [0xab; 32],
    }).unwrap());

    emit("nested_struct", &bcs::to_bytes(&NestedStruct {
        inner: InnerStruct { x: 100, y: 200 },
        flag: false,
    }).unwrap());

    emit("complex_struct", &bcs::to_bytes(&ComplexStruct {
        id: 999999,
    ...
    emit("with_option_some", &bcs::to_bytes(&WithOption {
        a: 10, b: Some(20), c: 30,
    }).unwrap());

    emit("with_option_none", &bcs::to_bytes(&WithOption {
        a: 10, b: None, c: 30,
    }).unwrap());

    // ── Enums ─────────────────────────────────────────────────────
    emit("enum_unit", &bcs::to_bytes(&TestEnum::Unit).unwrap());
    emit("enum_with_u64", &bcs::to_bytes(&TestEnum::WithU64(0x1234)).unwrap());
    emit("enum_with_bytes", &bcs::to_bytes(&TestEnum::WithBytes([0xAA, 0xBB, 0xCC, 0xDD])).unwrap());
    emit("enum_with_struct", &bcs::to_bytes(&TestEnum::WithStruct { a: 1000, b: 42 }).unwrap());

    // ── Tuples ────────────────────────────────────────────────────
    emit("tuple_i8_string", &bcs::to_bytes(&TupleStruct(-1, "diem".to_string())).unwrap());
    ...
    // ── Boundary values ───────────────────────────────────────────
    emit("u64_deadbeef", &bcs::to_bytes(&0xDEADBEEFCAFEBABEu64).unwrap());
    ...
    emit("sui_addr_sequential", &bcs::to_bytes(&{
        let mut a = [0u8; 32];
